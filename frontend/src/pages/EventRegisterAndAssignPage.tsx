...
// Modelo do usuário (mesmos campos da PublicRegisterPage)
type PublicUserForm = {
  username: string;
  email: string;
  password: string;
...
export default function EventRegisterAndAssignPage() {
  ...
  async function findUserIdByEmail(email: string) {
    try {
      ...
      const res = await api.get(`/users`, { params: { q: email }, headers });
      const list = res.data?.items ?? res.data ?? [];
      const exact = (list as any[]).find(u => u.email === email || u.nome === email);
      if (exact) return exact.id;
      if ((list as any[]).length === 1) return (list as any[])[0].id;
      return null;
    } catch {
      return null;
    }
  ...
  async function tryLinkToEvent(userId: number) {
    if (!paramEventId) return;
    if (!token) {
      // No token available to create worker link — inform user
      setSuccess(prev => (prev ? prev + " (Usuário criado; falta token para vincular ao evento)" : "Usuário criado; falta token para vincular ao evento"));
      return;
    }
    try {
      await api.post(`/event/${paramEventId}/workers`, {
        user_id: userId,
        role_name: null,
        days_assigned: null
      }, { headers: { Authorization: `Bearer ${token}` } });
      setSuccess(prev => (prev ? prev + " (Usuário criado e vinculado ao evento)" : "Usuário criado e vinculado ao evento"));
    } catch (err: any) {
      console.error("Falha ao vincular ao evento:", err);
      setError("Usuário criado, mas falha ao vincular ao evento: " + (err?.response?.data?.detail || err?.message || String(err)));
    }
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      try {
        console.debug("[EventRegister] formData avatar_file:", form.avatar_file);
        console.debug("[EventRegister] formData fd.get('avatar_file'):", formData.get("avatar_file"));
      } catch (e) {
        console.debug("[EventRegister] unable to debug FormData (browser limitation)");
      }
      ...
      // Try to extract created user id
      let newUserId: number | null = null;
      if (data?.id) newUserId = data.id;
      else if (data?.user?.id) newUserId = data.user.id;
      else if (data?.data?.id) newUserId = data.data.id;
      else {
        newUserId = await findUserIdByEmail(form.email);
      }

      if (paramEventId && newUserId) {
        // Attempt to link
        await tryLinkToEvent(newUserId);

        // NÃO navegar automaticamente para a página interna de equipe.
        // Só informar o usuário sobre o resultado.
        if (token) {
          setSuccess(prev => (prev ? prev + " (Usuário criado; vínculo ao evento tentado)" : "Usuário criado; vínculo ao evento tentado"));
        } else {
          setSuccess(prev => (prev ? prev + " (Usuário criado; falta token para vincular ao evento)" : "Usuário criado; falta token para vincular ao evento"));
        }
      } else if (paramEventId && !newUserId) {
        setError("Usuário criado, mas não foi possível obter o ID para vincular ao evento. Verifique manualmente.");
      }
    } catch (err: any) {
