...
export default function UserManagementTab() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // If there is a file selected, send multipart/form-data.
    const hasFile = !!form.avatar_file;

    if (hasFile) {
      const fd = new FormData();
      // append file under the key backend expects: "avatar_file" (used earlier in public register)
      if (form.avatar_file) fd.append('avatar_file', form.avatar_file);

      // append rest of fields as strings
      fd.append('nome', String(form.full_name || form.nome || form.username || ''));
      if (form.username) fd.append('username', String(form.username));
      if (form.email) fd.append('email', String(form.email));
      if (form.password) fd.append('password', String(form.password));
      if (form.cpf) fd.append('cpf', String(form.cpf));
      if (form.phone) fd.append('phone', String(form.phone));
      if (form.department) fd.append('department', String(form.department));
      if (form.institution) fd.append('institution', String(form.institution));
      if (form.birth_date) fd.append('birth_date', String(form.birth_date));
      if (form.notes) fd.append('notes', String(form.notes));
      if (form.address) fd.append('address', String(form.address));
      if (form.specialty) fd.append('specialty', String(form.specialty));
      if (form.role_id !== undefined && form.role_id !== null) fd.append('role_id', String(form.role_id));
      fd.append('accepted_terms', String(Boolean(form.accepted_terms)));
      fd.append('is_active', String(Boolean(form.is_active)));
      fd.append('is_admin', String(Boolean(form.is_admin)));

      const url = editUser ? `/users/${editUser.id}` : '/users/';
      const method = editUser ? api.patch : api.post;

      try {
        // DO NOT set Content-Type header; axios will set the correct multipart boundary
        await method(url, fd, { headers: { Authorization: `Bearer ${token}` } });
        setMessage(editUser ? 'Usuário editado com sucesso!' : 'Usuário criado com sucesso!');
        setForm({ ...initialForm });
        setEditUser(null);
        setOpen(false);
    ...
    const payload: any = {
      ...
      avatar_url: form.avatar_url, // existing URL option
    ...
    try {
      ...
      const detail = extractErrorMessage(err);
      setError(detail);
      console.error("Erro salvar usuário:", err);
      if ((err as any)?.response) console.debug("Server response:", (err as any).response.data);
    }
  }
  ...
  function handleEdit(user: UserForm) {
    setForm({
      ...
      password: "",
      avatar_file: null, // reset file input by default
    } as UserForm);
    setEditUser(user);
    setOpen(true);
    // if user has avatar_url, set preview to it
    if (user.avatar_url) {
